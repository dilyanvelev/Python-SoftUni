...
for _ in range(len(final_colors)):
    if final_colors[0] == 'orange':
        if 'red' not in final_colors and 'yellow' not in final_colors:
            final_colors.popleft()
        else:
            final_colors.popleft()
    elif final_colors[0] == 'purple':
        if 'red' not in final_colors and 'blue' not in final_colors:
            final_colors.popleft()
        else:
            final_colors.popleft()

    elif final_colors[0] == 'green':
        if 'blue' not in final_colors and 'yellow' not in final_colors:
            final_colors.popleft()
        else:
            final_colors.popleft()
    else:
